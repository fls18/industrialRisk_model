# -*- coding: utf-8 -*-
# industrialRisk_app.py — 입력 4종(지역/규모/업종/근속기간),
# 모델/룩업 예측 + 구간피드백 + 개선 Top3 + 체크리스트
# + 산업/규모/근속 기반 사전 가중(prior uplift) & 위험구간 재조정
...
MODEL_PATH = "industrial_risk_model.h5"
...
have_model = all(os.path.exists(p) for p in [MODEL_PATH, SCALER_PATH, NUMCOL_PATH, ONEHOT_PATH])
...
with c1:
    ...
    industry = st.selectbox("업종", INDUSTRY_OPTIONS, index=3)  # 디폴트 제조업
with c2:
...
# ---------------- 예측용 벡터 생성 ----------------
def make_num_vec():
    if not have_model or scaler is None or not NUM:
        return np.zeros((1, 0))
...
def make_cat_row(region_val, industry_val, scale_val, tenure_bucket_val):
    ...
    keys = [
        ...
        f"업종_{industry_val}",
        f"규모_{scale_val}",
        f"근속버킷_{tenure_bucket_val}",
    ]
    for k in keys:
        if k in Xcat.columns:
            Xcat.loc[0, k] = 1.0
    return Xcat
...
def predict_model(region_val, industry_val, scale_val, tenure_bucket_val):
    ...
    file_dummy_cols = [c for c in CAT if c.startswith("파일_")]
    if file_dummy_cols:
        preds=[]
...
def lookup_score(region_val, industry_val):
    ...
    if "업종" in df.columns and "지역" in df.columns:
        ...
        if sub.empty:
            sub = df[df["업종"].astype(str)==industry_val]
    else:
    ...
    if "가중사건" in sub.columns:
        n = _norm01(sub["가중사건"])
        if n is not None:
            return float(pd.to_numeric(n, errors="coerce").mean())
...
def apply_priors(p, industry_val, scale_val, tenure_years):
    """예측 p(0~1)에 산업/규모/근속 기간 가중을 가산(clip)."""
    u = 0.0
...
def pretty_scale(p):
    # 중앙 0.5 기준 완만 확대(0/1 포화 방지). priors 적용 후 마지막에 사용.
    return float(np.clip(0.5 + (p - 0.5) * 1.1, 0.0, 1.0))

# ---------------- 피드백/개선안/체크리스트 ----------------
def risk_band_msg(p):
    # 재조정: 🟢≤0.35, 🟠 0.35~0.65, 🔴>0.65
...
def checklist_ui():
    ...
    for i, item in enumerate(CHECKLIST_ITEMS):
        with cols[i % 2]:
            flag = st.checkbox(item, key=f"chk_{i}")
            done.append(flag)
...
if st.button("위험도 계산하기", use_container_width=True):
    ...
    pred = None
    try:
        pred = predict_model(region, industry, scale, bin_tenure_years(tenure))
    ...
    # 사전 가중 적용 → 마지막에 완만 스케일
    p_prior = apply_priors(float(pred), industry, scale, float(tenure))
    p = pretty_scale(p_prior)
    ...
    with st.expander("진단 경로/근거"):
        st.write(f"- 사용 경로: **{pathway}**")
        st.write("- 산업/규모/근속 기간에 따른 사전 가중(prior)을 적용해 현실성 보정.")
        st.write("- 모델이 파일 카테고리를 썼다면 파일_더미 평균화로 일반화.")
        st.write("- 숫자 특징은 학습 스케일러 기준으로 표준화.")
